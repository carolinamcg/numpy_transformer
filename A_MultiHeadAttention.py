...
class MultiHeadAttention(NNModule):
    ...
    def get_values_attention(self, Q, K, V):
        ...
        A = self.get_attention_weights(Q, K)
        H = np.matmul(A, V) #(num_heads, seq_length, self.d)
        return H, A
    ...
    def forward_crossattention(self, Henc, X):
        ...
        # Calculate the attention weights for each of the heads
        H_cat, A = self.get_values_attention(Q, K, V) #H_cat = (num_heads, seq_length, self.d)

        # Merge heads outputs into the last array's dimension
        H_cat = H_cat.transpose(0,2,1,3).reshape(bs, seq_length, -1) #(bs, seq_length, self.d * number_heads)
        
        # Final linear layer  
        H = np.matmul(H_cat.reshape(seq_length, -1), self.W_h) + self.bias_h # (seq_length, self.d_model)
